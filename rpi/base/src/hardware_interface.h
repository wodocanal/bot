...
public:
	HardwareInterface(ros::NodeHandle node, ros::NodeHandle private_node, double target_max_wheel_angular_speed);
...
HardwareInterface::HardwareInterface(ros::NodeHandle node, ros::NodeHandle private_node, double target_max_wheel_angular_speed)
	: _node(node)
	...
	_left_wheel_angle_sub = _node.subscribe("/left_wheel/angle", 1, &HardwareInterface::leftWheelAngleCallback, this);
	_right_wheel_angle_sub = _node.subscribe("/right_wheel/angle", 1, &HardwareInterface::rightWheelAngleCallback, this);
}

void HardwareInterface::writeCommandsToHardware() {
...
void HardwareInterface::updateJointsFromHardware(const ros::Duration& period) {
...
void HardwareInterface::leftWheelAngleCallback(const std_msgs::Float64& msg) {
	_left_wheel_angle = msg.data;
}

void HardwareInterface::rightWheelAngleCallback(const std_msgs::Float64& msg) {
	_right_wheel_angle = msg.data;
}

void HardwareInterface::limitDifferentialSpeed(double& diff_speed_left_side, double& diff_speed_right_side) {
